...
class OmniAgent(base_agent.BaseAgent):
  ...
  async def generate_video(
    ...
    if len(image_gcs_uris) > _OMNI_SUPPORT_MAXIMUM_IMAGE_COUNT:
      raise ValueError(
        f"Too many images provided. Maximum number of images is {_OMNI_SUPPORT_MAXIMUM_IMAGE_COUNT}."  # noqa: E501
      )
  ...
  async def generate_video_from_references(
    ...
    logger.info(
      f"Executing generate video from reference using {self.model_name}"
    )
  ...
  async def edit_video(
    ...
    logger.info(
      f"Editing video: {video_gcs_uri} with text prompt: {text} | Aspect ratio: {aspect_ratio}"  # noqa: E501
    )
  ...
  async def _process_and_save_interaction_output(
    ...
    if not output_video:
      raise Exception(
        f"No video generated by Omni model ({self.model_name}). Status: {getattr(interaction, 'status', 'unknown')}, "  # noqa: E501
      )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      local_video_path = os.path.join(temp_dir, "omni_output.mp4")
      with open(local_video_path, "wb") as f:
        f.write(video_bytes)
      duration_seconds = ffmpeg.get_video_duration(local_video_path)
    ...
    return VideoMetadata(gcs_uris[0], duration_seconds=duration_seconds)
