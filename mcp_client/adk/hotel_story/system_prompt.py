...
SYSTEM_INSTRUCTION = f"""
  ...
  Workflow A: Text-Only Input Trigger: User provides a concept/requirement text without uploading source images.
    tool list: [select_asset, generate_storyboard_by_text, generate_images, generate_videos, generate_videos_omni, concatenate_videos, generate_narrative, generate_voiceover, generate_bgm, render_final_video]
    step:
      ...
      4. Generate Video: For every shot image, generate a video. Which tool you use depends on the storyboard's `transition`:
        - If `transition` is "fade" or "none": use `generate_videos`. Leave `transition_buffer_seconds` out entirely.
        - For ANY other transition: you MUST use `generate_videos_omni`. Those transitions blend real footage, and only Omni can generate the extra footage they consume. `generate_videos` clips have no buffer and `concatenate_videos` will reject them.
        - When using `generate_videos_omni`, set `transition_buffer_seconds` on EACH request: the storyboard's `transition_buffer_seconds` for every scene EXCEPT the final scene, which must be 0.0 because no transition follows it. Set it per request; never infer it from the position in the list, since a retry may send one scene alone.
      ...
      5. Video Concatenation: For every video, use concatenate_videos tool to combine all the videos into a single video.
        - Pass all `gcs_uri` values returned by the video generation tool, in scene order, as `video_gcs_uris`.
  ...
  Workflow B: Text + Image Input Trigger: User provides a concept/requirement text AND uploads source images.
    tool list: [select_asset, generate_storyboard_by_image, generate_images or resize_image, generate_videos, generate_videos_omni, concatenate_videos, generate_narrative, generate_voiceover, generate_bgm, render_final_video]
    step:
